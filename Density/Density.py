...
class Density(Slide):
  ...
  def construct(self):
    ...
    for i in range(10):
      current_data = data[range(i+1)]
      print("Generating experiment ", i)
      self.play(UpdateFromFunc(updating_group, lambda dummy: update(dummy, current_data, i)), run_time=0.1)
      self.next_slide()
    self.next_slide()
    
    for i in range(1, 21):
      current_data = data[range(i*10)]
      print("Generating experiment ", i*10)
      self.play(UpdateFromFunc(updating_group, lambda dummy: update(dummy, current_data, i, jump=10)), run_time=0.1)
